...
void solve()
{
    ...
    // ai - can be negative so we take care of it;
    vector<ll> positive;
    vector<ll> negative;
    for (auto x : arr)
    {
    ...
    if (positive.size() == 0)
    {
        // means all numbers are negative;
        // -1 , -2 ,  -3 , -4 ..;
        ll nsize = negative.size();
        cout << negative[nsize - 1] * negative[nsize - 1] << " " << negative[0] * negative[0] << endl;
        return;
    }
    else if (negative.size() == 0)
    {
        // means all are positive; // 1 2 3  4 5 61
        ll psize = positive.size();
        cout << positive[0] * positive[0] << " " << positive[psize - 1] * positive[psize - 1] << endl;
        return;
    }
    else
    {
        if (abs(negative[0]) > positive[positive.size() - 1])
        {
            cout << positive[positive.size() - 1] * negative[0] << " " << negative[0] * negative[0] << endl;
        }
        else
        {
            cout << positive[positive.size() - 1] * negative[0] << " " << positive[positive.size() - 1] * positive[positive.size() - 1] << endl;
        }
    }
}
