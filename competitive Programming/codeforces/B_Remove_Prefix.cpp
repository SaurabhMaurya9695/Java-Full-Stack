...
void solve()
{
    ll n ;
    cin >> n ;
    vector<ll > skill = readvector(n);
    vector<ll >  dmg = readvector(n); 
    vector<pair<int, int >> b1 ;
    vector<pair<int,int>> b0 ;
    for(int i = 0 ; i< n ; i++)
    {
        if(skill[i] == 1)
        {
            b1.push_back({skill[i] , dmg[i]});
        }
        else{
            b0.push_back({skill[i] , dmg[i]});
        }
    }
    ll x = b1.size();
    ll y = b0.size();
    sort(b1.begin(), b1.end() , fun);
    sort(b0.begin(), b0.end() , fun);
    ...
    if(i == j)
    {
        while(i && j)
        {
            ans = ans + 2 * b1[j--].second + 2 * b0[i--].second;
        }
        if(b0[0].second > b1[0].second)
        {
            ans += 2 * b0[0].second + b1[0].second;
        }
        else{
            ans += b0[0].second + 2 * b1[0].second;
        }
        cout<< ans << endl;
        return ;
    }
