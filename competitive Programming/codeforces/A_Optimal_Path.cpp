...
#define print(v) for(int i = 0 ; i< n; i++){ cout<<v[i] << " ";}
...
int find(int r , int c , ll n , ll m , vector<vector<ll>> &arr , vector<vector<ll>> &dp)
{
    // base case ;
    if(r == n && c == m)
    {
    ...
    int right = find(r + 1 , c , n  , m , arr , dp) ;
    int dowm = find(r , c + 1 , n , m , arr , dp );
    return dp[r][c] = min(right , dowm) + arr[r][c] ;
...
void solve()
{
    ...
    // cout<< find(0 , 0 , n , m , arr , dp) << endl;

    // when we are go in the direction of top most to the right till m - 1 & from m to 2m , 3m, 4m, ...
    // this will give us a min sum  ;
    ll x = ((m * (m - 1) ) / 2) ;
    ll y = (m * (n * (n + 1) / 2));
    cout<< (x + y) << endl;
} 
